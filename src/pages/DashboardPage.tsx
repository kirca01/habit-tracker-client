...
export default function DashboardPage() {
  ...
  const fetchHabits = async () => {
    try {
      ...
      console.error('Greška pri dohvatanju navika');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = async (name: string, description: string, color: string) => {
    try {
      ...
      console.error('Greška pri dodavanju navike');
    }
  };
  ...
  const handleCheckIn = async (id: number) => {
    try {
      ...
      console.error('Greška pri check-in');
    }
  };
  ...
  const handleUndoCheckIn = async (id: number) => {
    try {
      ...
      console.error('Greška pri undo check-in');
    }
  };
    ...
    <div className="min-h-screen bg-gray-950 p-4 md:p-8">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-white">Moje navike</h1>
            <p className="text-gray-400 text-sm mt-1">
              {completedToday} / {habits.length} završeno danas
            </p>
          </div>
        ...
        {loading ? (
          <p className="text-gray-400 text-center mt-20">Učitavanje...</p>
        ) : habits.length === 0 ? (
          <div className="text-center mt-20">
            <p className="text-gray-400 text-lg">Nemaš još navika.</p>
            <p className="text-gray-600 text-sm mt-1">Klikni "Dodaj" da počneš!</p>
          </div>
        ) : (
